...
const Login = () => {
  ...
  const { data: oAuthData } = useFetchOauthDataQuery(ENDPOINTS.PROD_URL ?? '')

  const [passwordShown, setPasswordShown] = useState(false);

  const sId = oAuthData?.service_id
  const url = `${ENDPOINTS.OAUTH}/authorize?response_type=code&client_id=${sId}&redirect_uri=${ENDPOINTS.PROD_URL}`
